...
def relational_query_pictures():
    d=Drawing("tuple-join","Match the topic, then check availability","Jo's gardening request matches available books B1 and B4. Jo's astronomy request matches B2. Sam's gardening request matches B1 and B4. Lee's history request has no match.",446)
    d.text(162,86,"Request",24,anchor="middle",weight="700")
    d.text(484,86,"Available matches",24,anchor="middle",weight="700")
    for y,name,topic,books in [(133,"Jo","Gardening",["B1","B4"]),(208,"Jo","Astronomy",["B2"]),(283,"Sam","Gardening",["B1","B4"]),(358,"Lee","History",[])]:
        d.rect(29,y-28,268,56,PALE,"none",10,0)
        d.text(163,y+7,f"{name} · {topic}",23,anchor="middle",weight="600")
        d.line(309,y,388,y,True,dash=not books)
        for x,book in zip([435,552],books):d.label(x,y,book,89,PALE)
        if not books:d.text(489,y+7,"No match",25,anchor="middle",weight="700")
    d.text(320,425,"One request can match more than one book.",22,anchor="middle")
    d.save()
    d=Drawing("tuple-witness","Both properties must hold in one row","In a two-row catalog, B2 is astronomy and available, while B3 is gardening and unavailable. Each desired property appears somewhere, but no single row is both gardening and available.",353)
    for x,book,topic,ready in [(29,"B2","Astronomy","Yes"),(345,"B3","Gardening","No")]:
        d.rect(x,76,266,177,PALE,TEAL,12,2)
        d.text(x+133,112,book,28,anchor="middle",weight="700")
        d.text(x+133,153,topic,25,anchor="middle")
        d.text(x+133,190,f"Available: {ready}",24,anchor="middle")
        d.text(x+133,231,"Both tests: No",24,anchor="middle",weight="700")
    d.text(320,296,"Each property occurs somewhere.",24,anchor="middle")
    d.text(320,329,"No row has both.",26,anchor="middle",weight="700")
    d.save()
    d=Drawing("domain-slots","Connect records through a shared value","A request has reader Jo and topic Gardening. A book has ID B1, title Small Gardens, topic Gardening, and availability Yes. The same topic value fills the topic slot in both records; the output keeps Jo and B1.",402)
    d.text(31,84,"Request",24,weight="700")
    for x,w,heading,value in [(192,107,"Reader","Jo"),(449,183,"Topic","Gardening")]:
        d.text(x,85,heading,22,anchor="middle")
        d.label(x,128,value,w)
    d.line(449,158,449,197,True)
    d.text(462,186,"same value",21)
    d.text(31,204,"Book",24,weight="700")
    for x,w,heading,value in [(81,82,"ID","B1"),(248,222,"Title","Small Gardens"),(449,155,"Topic","Gardening"),(578,72,"Available","Yes")]:
        d.text(x,240,heading,21,anchor="middle")
        d.label(x,283,value,w)
    d.rect(66,333,508,47,GOLD,"none",12,0)
    d.text(320,364,"Keep reader and ID: (Jo, B1)",25,anchor="middle",weight="700")
    d.save()
    d=Drawing("domain-coverage","Does every request have a match?","Jo's Gardening and Astronomy requests are covered, Sam's Gardening request is covered, and Lee's History request is missing. Dee has no requests. The every-request condition passes Jo, Sam, and Dee; requiring at least one request removes Dee.",434)
    for x,label in [(68,"Reader"),(311,"Requested topics"),(557,"Every?")]:d.text(x,88,label,23,anchor="middle",weight="700")
    for y,name,topics,verdict in [(139,"Jo",["Gardening: covered","Astronomy: covered"],"Yes"),(215,"Sam",["Gardening: covered"],"Yes"),(277,"Lee",["History: missing"],"No"),(339,"Dee",["No requests"],"Yes")]:
        d.rect(22,y-34,596,65 if len(topics)>1 else 54,PALE if verdict=="Yes" else "#fbe6de","none",9,0)
        d.text(67,y+7,name,25,anchor="middle",weight="700")
        for j,topic in enumerate(topics):d.text(183,y-8+(j*28) if len(topics)>1 else y+7,topic,23)
        d.text(557,y+7,verdict,25,anchor="middle",weight="700")
    d.text(320,393,"Require a request too? Keep Jo and Sam.",23,anchor="middle")
    d.text(320,420,"Dee has no request that fails.",21,anchor="middle")
    d.save()

def communicating_process_pictures():
    d=Drawing("process-orders","Two preparations, one shared handoff","Starting with neither preparation done, folding makes only Jo ready and clearing makes only Sam ready. Either preparation can happen first. After both, the handoff can occur, followed by displaying the poster.",457)
    d.label(320,100,"Neither ready",211)
    d.label(141,213,"Jo ready",186)
    d.label(499,213,"Sam ready",186)
    d.label(320,326,"Both ready",207)
    for x1,y1,x2,y2 in [(251,129,161,177),(389,129,479,177),(161,241,251,290),(479,241,389,290)]:d.line(x1,y1,x2,y2,True)
    for x,y,label in [(161,142,"fold"),(479,142,"clear"),(145,282,"clear"),(495,282,"fold")]:d.text(x,y,label,23,anchor="middle")
    d.line(320,355,320,380,True)
    d.label(320,415,"Handoff, then display",334,GOLD)
    d.save()
    d=Drawing("process-deadlock","Agree on the next shared event","In the blocked version Jo's first event is receipt and Sam's first event is handoff, so no shared first event is possible. In the repaired version both begin with handoff and then receipt, so they can agree at each step.",443)
    for top,title,jo,sam,caption in [(76,"Blocked",["Receipt","Handoff"],["Handoff","Receipt"],"No shared first event"),(278,"Repaired",["Handoff","Receipt"],["Handoff","Receipt"],"Both events can now occur in order")]:
        d.text(30,top,title,26,weight="700")
        for y,name,events in [(top+39,"Jo",jo),(top+99,"Sam",sam)]:
            d.text(38,y+6,name,24,weight="700")
            for x,event in zip([246,512],events):d.label(x,y,event,177,PALE if top==278 else "#fbe6de")
            d.line(347,y-4,410,y-4,True)
        d.text(320,top+150,caption,23,anchor="middle",weight="600")
    d.save()

def ambient_pictures():
    d=Drawing("ambient-enter","Move a boundary with its contents","Before entry, bag and van are side by side, and parcel is inside bag. After bag enters van, parcel remains inside bag, and bag is inside van.",389)
    for x,label in [(162,"Before"),(478,"After")]:d.text(x,89,label,25,anchor="middle",weight="700")
    d.rect(32,118,145,204,PALE,TEAL,12,3)
    d.text(105,153,"bag",25,anchor="middle",weight="700")
    d.rect(48,232,113,60,GOLD,INK,8,2)
    d.text(105,269,"parcel",24,anchor="middle")
    d.rect(197,159,103,163,"white",INK,12,3)
    d.text(249,195,"van",25,anchor="middle",weight="700")
    d.line(310,216,340,216,True)
    d.rect(354,118,254,204,"white",INK,12,3)
    d.text(481,153,"van",25,anchor="middle",weight="700")
    d.rect(376,172,210,134,PALE,TEAL,10,3)
    d.text(481,208,"bag",25,anchor="middle",weight="700")
    d.rect(420,231,122,58,GOLD,INK,8,2)
    d.text(481,268,"parcel",24,anchor="middle")
    d.text(320,368,"Action inside bag: in van",24,anchor="middle")
    d.save()
    d=Drawing("ambient-exit-open","Leave the van, or remove the bag?","Starting from parcel inside bag inside van, exiting moves the intact bag beside the van. Opening bag instead removes its boundary, leaving parcel directly inside van. The two outcomes preserve the parcel but change different boundaries.",570)
    d.rect(170,75,300,178,"white",INK,12,3)
    d.text(320,108,"van",25,anchor="middle",weight="700")
    d.rect(197,126,246,109,PALE,TEAL,10,3)
    d.text(320,158,"bag",25,anchor="middle",weight="700")
    d.rect(244,172,152,46,GOLD,INK,8,2)
    d.text(320,203,"parcel",23,anchor="middle")
    d.line(241,266,157,331,True)
    d.line(399,266,483,331,True)
    d.text(95,292,"out van",24,anchor="middle",weight="700")
    d.text(531,292,"open bag",24,anchor="middle",weight="700")
    d.rect(27,397,112,115,"white",INK,12,3)
    d.text(83,434,"van",25,anchor="middle",weight="700")
    d.rect(169,355,138,157,PALE,TEAL,10,3)
    d.text(238,393,"bag",25,anchor="middle",weight="700")
    d.rect(182,442,112,50,GOLD,INK,8,2)
    d.text(238,475,"parcel",23,anchor="middle")
    d.rect(353,355,259,157,"white",INK,12,3)
    d.text(483,393,"van",25,anchor="middle",weight="700")
    d.rect(403,442,159,50,GOLD,INK,8,2)
    d.text(483,475,"parcel",23,anchor="middle")
    d.text(164,548,"Bag stays intact",23,anchor="middle",weight="600")
    d.text(483,548,"Bag boundary gone",23,anchor="middle",weight="600")
    d.save()

def spi_pictures():
    d=Drawing("spi-decrypt","Receiving is not yet reading","The same ciphertext C is tested with two keys. Decryption with the matching key K recovers print one poster and continues. A different key J does not enable that decryption step.",356)
    d.text(320,81,"C was encrypted with K",24,anchor="middle")
    for y,key,lines in [(145,"K",["Read:","print 1 poster"]),(257,"J",["No decryption","step"] )]:
        d.label(71,y,"C",74,GOLD)
        d.line(120,y-4,183,y-4,True)
        d.label(278,y,f"Try key {key}",158)
        d.line(371,y-4,414,y-4,True)
        d.rect(428,y-40,187,79,PALE if key=="K" else "#fbe6de",INK,12,2)
        for j,line in enumerate(lines):d.text(522,y-9+j*28,line,23,anchor="middle",weight="600")
    d.text(320,334,"Only the matching key enables this rule.",23,anchor="middle")
    d.save()
    d=Drawing("spi-replay","Copying a message can repeat an action","A printer that remembers no earlier requests accepts ciphertext C twice and prints two posters. A printer that checks a matching one-use challenge accepts the first valid response, marks the challenge used, and rejects a repeated response, printing one poster.",434)
    for y,title,second,total in [(133,"Without a replay record","Accept",2),(315,"With a one-use challenge","Reject",1)]:
        d.text(29,y-53,title,25,weight="700")
        for x,result in [(103,"Accept"),(338,second)]:
            d.label(x,y,"C",97,GOLD)
            d.text(x,y+56,result,24,anchor="middle",weight="700")
        d.line(164,y-5,271,y-5,True)
        d.text(218,y-23,"then",22,anchor="middle")
        d.text(527,y+3,total,34,anchor="middle",weight="700")
        d.text(527,y+38,"posters" if total==2 else "poster",25,anchor="middle")
        if total==1:d.text(218,y+37,"mark used",21,anchor="middle")
    d.text(320,414,"The repeated response has the same challenge.",22,anchor="middle")
    d.save()
...
def main():
    ...
    relational_query_pictures(); communicating_process_pictures(); ambient_pictures(); spi_pictures()
