...
| Tuple and domain relational calculus | Two presentations with equal expressive power under safety restrictions | Whole-record variables can be replaced by variables for their fields; both express existential and universal queries | [Tuple](tracks/data/tuple-relational-calculus.md) and [domain](tracks/data/domain-relational-calculus.md) tracks; *Database System Concepts* §27.3 |
| Relational calculus and relational algebra | Expressive equivalence in the classical setting | Safe condition-based queries have operation-based expressions with the same result; execution strategy and SQL extensions are separate issues | [Data tracks](tracks/data/tuple-relational-calculus.md); *Database System Concepts* §27.3 |
| CCS, CSP, and ACP | Comparison of process formalisms | Each can model a joint handoff, but synchronization, blocking of unpaired actions, and hiding use different mechanisms | [Coordination track](tracks/interaction/ccs-csp-acp.md); Hoare, Bergstra–Klop, and van Glabbeek |
| Pi and spi calculi | Extension with cryptographic operations | Name-passing process descriptions gain operations for encrypted messages and protocol reasoning | [Locations and messages](tracks/interaction/ambient-and-spi-calculi.md); Abadi–Gordon |
...
The library's “some match” and “every request covered” questions share a catalog, but their quantifiers differ. Both tuple and domain calculus can express either question. The [ambient and spi track](tracks/interaction/ambient-and-spi-calculi.md) shares a delivery setting while changing from nested locations to symbolic messages; it does not identify moving a boundary with decrypting a message.
